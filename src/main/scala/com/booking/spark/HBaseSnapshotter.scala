...
import org.apache.spark.sql.types.{DataType, DateType, DoubleType, IntegerType, LongType, StringType, StructField, StructType, TimestampType}
...
object HBaseSnapshotter {
          ...
          try {
            ...
            if (fieldValue.toUpperCase() == "NULL")
              ...
              field.dataType match {
                ...
                case _ => Bytes.toString(familyMap.get(Bytes.toBytes(field.name)))
              }
            }
          }
          catch {
  ...
  def main(args: Array[String]): Unit = {
    ...
    conf.setIfMissing("hive.execution.engine", "spark")
   ...
   // val sqlContext = new SQLContext(sc)
    ...
    //dataFrame.registerTempTable("wikistats")
    //val group_res = sqlContext.sql("select * from wikistats limit 10")
    //group_res.show()
